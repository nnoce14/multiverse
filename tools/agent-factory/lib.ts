...
export const scoreWeights = {
  business_truth_alignment: 10,
  architecture_boundary_integrity: 10,
  provider_contract_stability: 9,
  refusal_safety: 10,
  developer_workflow_fit: 8,
  testability_and_tdd_readiness: 9,
  implementation_sequence: 8,
  maintenance_cost: 6,
  release_surface_control: 6,
  evidence_discipline: 9
} as const;
...
export async function globFiles(patterns: string | string[]): Promise<string[]> {
  return fg(patterns, {
    ...
    dot: true,
    onlyFiles: true,
    ignore: ["node_modules/**", "**/node_modules/**", ".git/**", ".codex/**"]
  });
}
...
export function compositeScore(scorecard: Scorecard): number {
  const scores = Object.keys(scoreWeights).map(
    (dimension) => scorecard.dimensions[dimension as ScoreDimension]?.score ?? 0
  );
