...
const restrictedTypes = new Set(["vendor", "historical-project", "internal-artifact"]);
const forbiddenPhrases = ["proven production ready", "1.0 complete", "security guaranteed"];
...
export async function checkSourceLedger(input: SourceLedgerCheckInput = {}): Promise<SourceLedgerCheckResult> {
  ...
  const schema = JSON.parse(await readFile(input.schemaPath ?? "schemas/source-record.schema.json", "utf8"));
...
async function main(): Promise<void> {
  const result = await checkSourceLedger({
    requiredSourceIds: ["src-openclinxr-agent-workflow-reference", "src-multiverse-old-reference"]
  });
