...
export function buildEvidenceDebtReport(
  scorecardCount: number,
  allDebt: EvidenceDebtRecord[]
): EvidenceDebtReport {
  ...
  return {
    ...
    closed_count: sortedDebt.filter(
      (debt) => debt.status === "closed" || debt.status === "resolved"
    ).length,
...
function compareDebt(left: EvidenceDebtRecord, right: EvidenceDebtRecord): number {
  return (
    left.iteration_number - right.iteration_number ||
...
function comparePriorityDebt(left: EvidenceDebtRecord, right: EvidenceDebtRecord): number {
  return (
    right.iteration_number - left.iteration_number ||
...
function countBy(
  records: EvidenceDebtRecord[],
  key: (record: EvidenceDebtRecord) => string
